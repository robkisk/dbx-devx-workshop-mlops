# Pet Retail Customer Churn MLOps Demo — Design Spec

## Overview

A customer churn prediction demo for pet retail, showcasing end-to-end MLOps on Databricks using the deploy code pattern. Built with Databricks Asset Bundles, GitHub Actions CI/CD, MLflow, Unity Catalog, Model Serving, and Data Profiling.

**Use case:** Predict whether a Pet retail customer will churn based on behavioral and transactional features (subscription status, order frequency, pet type, days since last order, etc.).
...
│   ├── pet_churn_inference.job.yml             # Batch inference job
│   ├── pet_churn_monitoring.job.yml            # Data Profiling refresh
│   ├── pet_churn_artifacts.yml                 # MLflow experiment + registered model in UC
│   └── pet_churn_wheel_demo.job.yml            # Wheel packaging demo job
├── src/
...
│   └── pet_churn_wheel/                        # Python wheel demo
│       ├── pyproject.toml
│       └── pet_churn_pkg/
│           ├── __init__.py
│           └── predict.py
...
- Notebook: `src/pet_churn/04_deploy_model.py`
...
- Produces `pet_churn_predictions_profile_metrics` and `pet_churn_predictions_drift_metrics`
...
Generates synthetic Pet retail customer data using the databricks-data-generation skill pattern. Writes to `{catalog}.{schema}.pet_churn_customers`.
...
- Reads `pet_churn_customers`
- Selects/renames feature columns
- Writes to `{catalog}.{schema}.pet_churn_features` as a Delta table, then sets primary key via `ALTER TABLE ADD CONSTRAINT pk PRIMARY KEY (customer_id)` (making it a UC feature table)
- Splits a held-out evaluation set to `{catalog}.{schema}.pet_churn_eval` for Champion/Challenger comparison
...
- Writes test split to `{catalog}.{schema}.pet_churn_test` for downstream validation
...
- Loads model from UC
- Reads test split from `{catalog}.{schema}.pet_churn_test` (written by train task)
...
- Checks if `@champion` alias exists:
  - **Yes:** loads both, evaluates on `pet_churn_eval` table, compares F1 scores. If challenger >= champion, promote challenger to `champion`. Otherwise keep existing champion and delete `challenger` alias.
...
- Writes to `{catalog}.{schema}.pet_churn_predictions` with columns: `customer_id`, `prediction`, `prediction_proba`, `model_version`, `timestamp`

### 06_monitor.py

- Uses `import databricks.lakehouse_monitoring as lm` (standalone package, available on ML Runtime)
- Creates (or gets existing) InferenceLog profile on `pet_churn_predictions`:
...
## Wheel Demo (pet_churn_wheel/)

A minimal Python package to demonstrate DABs wheel task packaging:

- `pet_churn_pkg/predict.py` contains a `score_batch(spark, model_uri, input_table)` function
- `pyproject.toml` defines the wheel build
- A small job resource (`pet_churn_wheel_demo.job.yml`) shows a `python_wheel_task` configuration
...
[04_deploy_model] -> @champion alias (if beats existing champion)
       ...
       |
[05_batch_inference] -> pet_churn_predictions (scored data)
       |
[06_monitor] -> pet_churn_predictions_profile_metrics
              -> pet_churn_predictions_drift_metrics
...
- Manual validation during workshop: `databricks bundle run pet_churn_training --target dev`
...
4. Show `pet_churn_training.job.yml` — explain the 3-task workflow
